...
class Irc:
	...
	# to add hooks, do on_channel_msg.registerHandler(method) etc
	...
	def mainloop(self):
		...
		while True:
			...
			if msg == 'JOIN':
				...
				self.on_join.call(params[0],user)
			...
			if msg == 'PART':
				...
				self.on_part.call(user,params[0])
...
#>> :Krenair!~Krenair@ZNC.MonsterProjects.org KICK #tf2 Riking :Riking
			...
			if msg == 'NOTICE':
				user = sender[0:sender.find('!')]
				# notice in a channel or private message?
				if params[0][0] == '#':
